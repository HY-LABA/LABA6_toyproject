...
├── algorithm.md                  # 궤적 추정 수식, 제어 루프, 검증된 성능, 파라미터
├── system_flow.md                # (구) 2026-08-19 시스템 흐름 — 지금은 이 문서와 algorithm.md 가 대체
├── docs/                         # 설계 근거 — "왜 그 값인가" + 미해결 리스크(open-questions.md)
│
...
| `control.py` | 착지점(트랙 원점 기준) + 트랙 시작 시점 오도메트리 = **월드 좌표 목표점**. 남은시간을 `DRIVE_AGGRESSION` 으로 나눠 보내고, `MIN_TARGET_DIST_M` 하한을 적용한다. 남은 거리·속도 계산은 피코가 한다. `max_body_speed` 는 피코와 같은 식. `python control.py` 로 `pico/config.h` 와 설정 대조 |
