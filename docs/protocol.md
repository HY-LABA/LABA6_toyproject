...
> **⚠ 필드 이름을 반드시 확인하고 쓸 것.** 12 B `<fff>`라는 형태는 예전 설계
> (`target_x, target_y, drive_time_s`)와 **크기가 같다.** 한쪽만 옛 의미로 해석하면
> 오류 없이 조용히 엉뚱한 속도로 돈다. 경위는 [CHANGELOG](../CHANGELOG.md) 참고.

**속도 계산은 전체 그림을 아는 파이5가 한다.** 피코는 궤적도 착지점도 모르므로
"0.3 m를 0.1 s에 가라"는 명령이 물리적으로 가능한지 판단할 수 없다.
