...
| 4 | Raspberry Pi Pico | 26 GPIO, 16 PWM channels, 3 g | 1 | 5,940 | Real-time motor control. See [why a separate MCU](../design/pico-control.md) |
...
lens you end up with** ([`calibrate.py`](../../src/pi5/prep/calibrate.py)). The
intrinsics in [`config.py`](../../src/pi5/config.py) are ours and will not match your
lens.
...
> [`physics.md` §3](../design/physics.md). **Use the FIT0186 numbers** — they are what
...
reaches top speed. **Acceleration wins.** See [`physics.md`](../design/physics.md).
...
the encoder is fine, the Pico dies. Details in [`hardware.md`](../design/hardware.md).
...
`CATCH_HEIGHT_M` in [`config.py`](../../src/pi5/config.py). **Measure yours and put
them in.** With the offset left at zero the robot parks 16 cm off target on
every single throw.

Sizing rationale is in [`physics.md` §6](../design/physics.md).
...
| AI HAT+ / Hailo-8L | Yes | Any accelerator, or CPU inference if you accept fewer frames. Fewer observations means a worse fit — see [`algorithm.md`](../design/algorithm.md) |
...
mean — are in [`hardware.md`](../design/hardware.md). **Read that before you glue
