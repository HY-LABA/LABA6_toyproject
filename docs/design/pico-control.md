...
| 판 | 위치 | 쓰는 때 |
|---|---|---|
| C (Pico SDK) | [`src/pico/`](../../src/pico/) | 기본. `src/pico/build/moving_trash_bin.uf2` 를 BOOTSEL 드래그로 굽는다 |
| MicroPython | [`src/pico_micropython/`](../../src/pico_micropython/) | 값을 자주 바꿔가며 튜닝할 때. 굽기·업로드 절차와 C 와의 차이 4군데는 [그 폴더 README](../../src/pico_micropython/README.md) |
...
([`src/pico_micropython/README.md` 3장](../../src/pico_micropython/README.md)).
...
MicroPython 판은 파일이 1:1 대응한다 ([`src/pico_micropython/README.md` 1장](../../src/pico_micropython/README.md)).

---

## 8. 파라미터

`src/pico/config.h` (MicroPython은 `src/pico_micropython/config.py`, **값을 고치면 둘 다** 고친다).
...
파이5 `src/pi5/config.py` 에도 있다. `python src/pi5/control.py` 가 `src/pico/config.h` 를 직접 읽어
...
1. **빌드·굽기** — C: `src/pico/build` 에서 `ninja` → `moving_trash_bin.uf2` 를 BOOTSEL 드래그.
   MicroPython: [README 2장](../../src/pico_micropython/README.md)
...
4. **거리·최대속도·정지거리** — 줄자와 대조:
   ```bash
   python src/pi5/teleop_test.py --port /dev/ttyACM0 --go-distance 1.0
