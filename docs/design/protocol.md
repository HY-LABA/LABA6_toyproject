...
구현: [`../pi5/communication.py`](../../src/pi5/communication.py)
↔ [`src/pico/communication.c`](../../src/pico/communication.c)
(MicroPython 판: [`src/pico_micropython/communication.py`](../../src/pico_micropython/communication.py) — 프레임 포맷 동일)
...
> ([`src/pico_micropython/README.md` 2-4장](../../src/pico_micropython/README.md)).
...
> ([algorithm.md 5장](algorithm.md#5-제어-루프-srcpi5mainpy-controlpy)). 피코는 받은 값을
...
> (`src/pi5/pico_test.py`, `src/pi5/teleop_test.py`)과 6장의 디코더뿐이다. 만들 때의 목록으로 둔다.
...
`src/pi5/communication.py` 의 `debug_decode(raw)` 가 프레임 하나를 사람이 읽는 텍스트로 바꾼다.
페이로드 길이(24/16/12 B)로 종류를 구분한다.

```
$ python src/pi5/communication.py < frame.bin
