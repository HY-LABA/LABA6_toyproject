...
```bash
python pi5/prep/check_setup.py
```
...
Copy everything in `pico_micropython/` to the board with Thonny. See
...
python pi5/control.py    # compares pi5/config.py against pico/config.h
```
...
```bash
python pi5/prep/calibrate.py capture --camera gs     # ~20 images, varied angles
python pi5/prep/calibrate.py solve --model fisheye   # or --model pinhole
```
...
> See [`docs/vision-pipeline.md`](vision-pipeline.md).
...
| Moves **exactly opposite** | All three motor signs inverted. Flip `MOTOR_SIGN` in `pico/config.h` and reflash, **or** set `DRIVE_INVERT = True` in `pi5/config.py` |
...
```bash
python pi5/pico_test.py --port /dev/ttyACM0 --all --dist 0.5 --speed 0.3
```
...
```bash
python pi5/measure_camera_yaw.py live       # needs a display
python pi5/measure_camera_yaw.py capture    # headless: saves annotated frames
```
...
[`docs/vision-pipeline.md`](vision-pipeline.md) covers dataset collection,
labelling, training and Hailo compilation — or point at any single-class
detector you have:

```bash
TRASH_HEF=/path/to/model.hef python pi5/main.py
```

Four collection paths are provided in [`pi5/prep/`](../pi5/prep/), differing
...
```bash
python pi5/test_accuracy.py --throws 10
```
...
```bash
python pi5/test_accuracy.py --replay logs/accuracy/*.json --span 0.25
```
...
[`docs/open-questions.md`](open-questions.md) (Korean).
