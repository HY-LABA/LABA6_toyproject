...
`(u₀, v₀)`는 **주점(principal point)**이며 대략 이미지 중심 (728, 544)이다.
정확한 값은 캘리브레이션으로 구한다.
...
떨어지면 z 추정이 주기적으로 흔들린다. 대책:

- bbox의 **짧은 변**을 물체의 짧은 치수에 매칭한다 (가장 안정적인 축)
...
결정한다 ([physics.md 6.3장](physics.md#63-x-y-오차는-z보다-훨씬-작다)).
...
문제는 **3프레임 차분으로 구한 vz의 오차가 1.1 m/s**라는 것이다. vz 자체가 수 m/s인데
오차가 그 수준이면 착지 예측이 성립하지 않는다.

같은 관측을 비행 전체에 걸쳐 중력 제약 하에 피팅하면 오차가 **0.087 m/s로 12배** 좋아진다.
계산 근거는 [physics.md 6장](physics.md#6-속도-추정-오차--설계를-바꾼-계산).
...
- **예측 모델:** 등가속 `(0, 0, −g_eff)` — `g_eff`는 클래스별
...
공기저항 모델 오차를 흡수한다. 종이컵 > 캔 ≈ 페트병 순으로 크게 잡는다.
...
`Z_CATCH`는 **통 입구의 높이**다. 카메라를 원점으로 잡았으므로 카메라 평면 위 통 입구까지의
높이(예: 0.15 m)를 넣는다.
...
**왜 이렇게 단순해도 되는가:** 30 Hz로 매번 다시 계산하므로 일종의 비례 유도(proportional
...
`|d_world| > d_max(T)`([physics.md 2장](physics.md#2-진짜-제약은-가속도다)의 공식)이면
...
| 값 | 방법 |
|---|---|
| `FOCAL_LENGTH_PX` | 체커보드 카메라 캘리브레이션 |
...
| 값 | 임시값 | 확정 시점 |
|---|---|---|
| `V_MAX` | 1.2 m/s | 피코 브링업 |
| `A_MAX` | 3.0 m/s² | 피코 브링업 |
