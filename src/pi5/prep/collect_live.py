...
⚠ 안전 — 장애물 감지가 없다
---------------------------
`docs/design/open-questions.md` 대로 이 시스템은 바닥이 비어 있다고 가정한다. 이 스크립트도
마찬가지다. 주변을 치우고, 사람이 옆에서 지켜보는 상태에서만 돌릴 것.
  ...
  · 가감속은 `--accel` 로 완만하게 준다 — 급가속은 바퀴를 미끄러뜨리고, 슬립은
    오도메트리를 오염시킨다 (`docs/design/open-questions.md`)
...
class Roamer:
    ...
    바퀴를 헛돌게 하고, 그 슬립이 오도메트리를 오염시킨다 (`docs/design/open-questions.md`).
    """
