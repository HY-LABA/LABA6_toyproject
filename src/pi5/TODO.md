...
> 리스크 분석은 [`../docs/design/open-questions.md`](../../docs/design/open-questions.md), 동작 설명은
> [`../docs/design/algorithm.md`](../../docs/design/algorithm.md).
...
- [ ] **합성 투척 성능 표 재실행** — [`docs/design/algorithm.md` 6장](../../docs/design/algorithm.md#6-검증된-성능) 표가
...
- [ ] **펌웨어 상수 대조** — `verify_wheel_config()` 는 `src/pico/config.h` 소스만 본다. 굽지 않은
      변경이나 MicroPython 판을 못 잡는다
- [ ] (선택) **ROI 크롭 추론** — 지금은 매 프레임 640 레터박스(2 m에서 물체 11 px).
      `tracker.predict_uv()` 주변만 원본 해상도로 자르면 축소가 없어진다
- [ ] (정리) `run.log` 가 루트에서 git 추적 중이다. `.gitignore` 에는 `src/pi5/run.log` 만 있고,
...
| `control.py` (직접 실행) | — | — | pi5 ↔ `src/pico/config.h` 바퀴 설정 대조, 방향별 최대 body 속도 표 |
