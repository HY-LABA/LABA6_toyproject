...
`src/pico/` 의 C 펌웨어를 MicroPython 으로 그대로 옮긴 것이다. 프로토콜·기구학·PID·
...
|---|---|---|
| `config.py` | `src/pico/config.h` | 값 동일. ⚠ pi5 `control.verify_wheel_config()` 는 여전히 **`src/pico/config.h` 를 읽는다** — 값을 고치면 양쪽 다 고칠 것 |
...
3. ⚠ **보드 종류를 꼭 확인할 것.** 이 프로젝트는 `pico2_w` = RP2350 이다
   (`src/pico/CMakeLists.txt` 의 `set(PICO_BOARD pico2_w)`). RP2040 용 UF2 를 구우면 안 올라간다.
...
포트는 그대로 `/dev/ttyACM0`, 115200 (`src/pi5/config.py`). MicroPython 도 USB CDC 로
잡히므로 바뀌는 게 없다. 두 가지만:
