...
**이 프로젝트는 카메라가 천장을 본다. 천장에는 조명이 있다.**
그래서 화면 안에 아주 밝은 광원이 들어오고, 자동노출이 **그 광원에 맞춰** 노출을 줄인다.
결과적으로 정작 보고 싶은 물체가 까맣게 나온다 — 역광 인물이 실루엣이 되는 것과 같다.

### 원인은 둘이고 해법이 정반대다

| | 증상 | 해법 |
|---|---|---|
| ⓐ **AE가 조명에 속았다** | 노출 여유가 남았는데 어둡다 | **EV를 올린다** |
| ⓑ **빛이 진짜 부족하다** | 노출·게인이 이미 한계다 | 소프트웨어로는 못 고친다. 조명을 켜라 |

구분하지 않고 EV만 계속 올리면 노이즈만 커진다. 그래서 `camera.py`의 `diagnose()`가
**실제 적용된 노출값을 읽어서** 둘을 갈라준다. `check_setup.py`와 `capture_dataset.py`가
시작할 때 자동으로 출력한다. 추측하지 말고 그 출력을 볼 것.

> 노출을 못 읽는 백엔드(일부 USB 웹캠)에서는 "가를 수 없다"고 솔직히 말하고
> 시도 순서만 알려준다.

### 실행 중에 바로 고치기

`capture_dataset.py`가 도는 동안:

| 키 | 하는 일 |
|---|---|
| `]` `[` | **EV 올림/내림** — 자동노출에서 가장 먼저 쓸 것 |
| `+` `-` | 수동 노출 늘림/줄임 (자동노출이 꺼진다) |
| `A` | 자동/수동 노출 토글 |
| `B` | **화면만** 밝게 (저장 이미지는 그대로) |

화면 아래에 `exp / gain / EV / mean`이 항상 표시된다. **mean 60 이상**이 목표다.

명령줄로도 같다: `--ev 2.5`, `--exposure 8000 --gain 4`, `--auto-exposure`.

### 두 가지 함정

- **`B`(preview boost)는 눈으로 볼 때만이다.** 저장 이미지와 MOG2 입력은 원본 그대로다.
  보정된 프레임을 MOG2에 넣으면 어두운 영역 노이즈까지 증폭돼 헛 덩어리가 늘어난다.
  화면이 밝아 보인다고 문제가 해결된 게 아니다 — `mean` 숫자를 봐라
- **어두운 데이터는 나중에 못 살린다.** 수집이 끝나면 어두웠던 프레임 비율을 알려주고,
  `sessions.json`에 노출 설정을 함께 기록한다. 절반 이상 어두웠다면 다시 찍는 게 맞다

### 그래도 어두우면

`gs` 프로파일은 노출이 800 µs다 (모션 블러를 막으려면 짧아야 한다). 이건 **원래 어둡고**,
게인으로도 한계가 있다. 그래서 설계 단계에서 **노출과 동기된 스트로브 LED**를 답으로
잡아뒀다 ([../docs/hardware.md 3.1장](../docs/hardware.md)). GS 카메라가 오면
이 문제가 다시, 더 크게 온다.
