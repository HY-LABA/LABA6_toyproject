...
┌─ ⚠ 렌즈 표기 화각은 과장이었다 (실측으로 확인) ────────────────────────────┐
│ 번들 ZH3019-14 표기는 **2.8mm, D=148° / H=118°** 인데, 실측은 전혀 다르다. │
│                                                                          │
│   2026-08-19 체커보드 캘리브레이션 (23장, RMS 0.411px, 9/9 커버)          │
│     fx 973  fy 974  cx 657  cy 535                                       │
│     대각 **108.8°** / 수평 89.3° / 수직 66.9°                             │
│     실효 초점거리 973 x 3.45µm = **3.36 mm**                              │
│                                                                          │
│ 줄자 검증: 벽에서 1.02m, 화면 가로 실폭 측정 2.20m.                        │
│   캘리브레이션 예측 2.03m (+8%)  vs  표기 역산 예측 2.94m (-25%).          │
│   표기값은 74cm나 빗나가 측정 오차로 설명이 안 된다.                       │
│                                                                          │
│ 예전에 이 자리에는 표기 사양에서 역산한 f_px 755 / 대각 138° 가 적혀        │
│ 있었다. **29% 틀린 값이었으니 다시 역산하지 말 것.** 저가 M12 렌즈가        │
│ 화각을 부풀려 표기하는 건 흔한 일이다.                                     │
│                                                                          │
│ 모델은 그대로 cv2.fisheye (r = f·θ_d) 를 쓴다. 실측 계수로 계산한 θ_d/θ 는 │
│ 화면 안에서 0.96~0.99 라 거의 등거리다.                                    │
└──────────────────────────────────────────────────────────────────────────┘
...
SPECS: dict[str, CameraSpec] = {
    ...
    "gs": CameraSpec(
        ...
        note="라즈베리파이 공식 GS 카메라와 동일 스펙(제조사가 호환품으로 표기). "
             ...
             "표기 화각 D=148°/H=118°는 과장이다 — 2026-08-19 실측은 "
             "대각 108.8°, f_px 973 (실효 3.36mm). 표기에서 역산하지 말 것. "
