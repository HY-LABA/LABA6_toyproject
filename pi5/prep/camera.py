...
SPECS: dict[str, CameraSpec] = {
    ...
    "imx219m12": CameraSpec(
        name="Pi 8MP IMX219 + M12 렌즈 LS40136 (B0103)", width=1640, height=1232, fps=40,
        calib_model="pinhole", exposure_us=2000, gain=4.0,
        max_exposure_us=3000, max_gain=16.0,
        note="1640x1232는 2x2 비닝 모드 — 풀해상도(3280x2464)는 15fps밖에 안 나오고 "
             "비닝은 40fps까지 나온다. 궤적 피팅은 시간 스팬이 생명이라 fps를 택했다. "
             "⚠ 롤링 셔터다: 낙하 물체의 위아래 줄이 다른 시각에 읽혀 bbox가 세로로 "
             "밀린다. 다만 이제 궤적 추정에 쓰는 건 bbox '중심'뿐이라 폭 왜곡보다 훨씬 "
             "덜 해롭다. ⚠ LS40136은 광각 M12 렌즈다 — 화각을 제품 스펙에서 확인해서 "
             "90°를 넘으면 calib_model을 fisheye로 바꿀 것. 그리고 f_px/주점은 반드시 "
             "체스보드 캘리브레이션으로 재서 pi5/config.py에 넣어야 한다 "
             "(지금 config.py 값은 센서 기하로 계산한 placeholder).",
    ),
    # PC 웹캠 (코드 점검용) ────────────────────────────────────────────────
    "webcam": CameraSpec(
