...
cv2.fisheye 가 쓰는 모델이 이걸 그대로 기술한다.  근거: ../docs/physics.md 7.2장

⚠ 예전에는 사양의 D=148°/H=118° 비율로 **왜곡 없는 순수 등거리**를 가정해
  f_px 755 를 역산했는데, 실제로는 위처럼 압축이 있어서 틀렸다. 압축이 있으면
  f_px 가 크면서도 화각이 넓다 — 973 과 133.5° 는 모순이 아니다.
  표기 화각에서 f 를 역산하지 말 것.
