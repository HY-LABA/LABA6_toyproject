...
> 2026-09-15 갱신. **지금 남은 것만** 적는다. 제어 설계는 [`../docs/pico-control.md`](../docs/pico-control.md),
> 통신 계약은 [`../docs/protocol.md`](../docs/protocol.md), 경위(버그 수정 이력)는 [`../CHANGELOG.md`](../CHANGELOG.md).
>
> 펌웨어가 두 판이다 — C(`pico/`, UF2) 와 MicroPython(`pico_micropython/`, Thonny).
> 값을 바꾸면 **`pico/config.h`, `pico_micropython/config.py`, `pi5/config.py` 셋을 같이** 본다.
...
- [x] 실물 로봇 구동 — `pi5/main.py` 목표점 명령으로 주행 확인 (2026-09-09~)
...
- [ ] **최대속도** — `python pi5/teleop_test.py --port /dev/ttyACM0 --speed-test 1.8 --speed-test-duration 1.0`
- [ ] **가속시간·정지거리** — `python pi5/teleop_test.py --port /dev/ttyACM0 --go-distance 1.0` /
...
- [ ] (정리) `pi5/control.verify_wheel_config()` 는 `pico/config.h` 만 읽는다 — MicroPython 판과
      실제로 구운 UF2 는 대조하지 않는다

## 참고

- 빌드: VS Code Pico 확장(SDK 2.3.0), `cmake -G Ninja` + `ninja` → `pico/build/*.uf2`. BOOTSEL 로 드래그
