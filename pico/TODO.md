...
파이5가 이제 **월드 좌표 도착 지점**(16 B)을 보낸다. 계약 전문은
[`../docs/protocol.md`](../docs/protocol.md) 2장.

- [ ] `communication.c` — 16 B 프레임 수용. 지금은 `RX_WAIT_LEN`에서
      `rx_len == SEND_PAYLOAD_LEN`(12)만 받고 나머지는 버린다. 그래서 **지금 펌웨어에
      목표점 명령을 보내면 조용히 틀리지 않고 그냥 안 먹는다** — 그건 좋은 성질이니
      바꿀 때도 유지할 것. `rx_payload[]`를 16으로 키우고 `12 || 16` 둘 다 받게 한다
- [ ] `TargetCommand` 처리 — `남은거리 = target − 자기 pose`, `v = 남은거리 ÷ 남은시간`.
      **파이가 이미 간 거리를 빼서 보내지 않으므로 여기서 한 번만 뺀다**
- [ ] 12 B 속도 명령 경로는 **그대로 유지** — `teleop_test.py`와 정지(STOP)가 쓴다.
      아래 2번 실측이 여기에 걸려 있다
- [ ] `POSITION_TOLERANCE_M` 도착 판정 (파이5 `config.py`와 같은 값, 지금 0.02)
