...
- 글로벌 셔터 카메라 (Sony IMX296, 최대 60fps, 위쪽 촬영)
  + 번들 **M12 2.8mm F2.2 어안** (대각 140°) — `CAMERA_MODEL = "fisheye"`
- 모터 내장 엔코더 3개 (홀센서, A/B 2상, 16 CPR, **VCC 3.3~5V → 3.3V에 물릴 것**)

### 구동부

- **FIT0186** 12V DC 기어드모터 3개 (251 RPM, 스톨 18 kgf·cm, 기어비 43.8:1)
- BTS7960 모터드라이버 3개
...
> **⚠ 카메라 장착 위치가 미결이고, 그것이 코드 결함과 얽혀 있다.**
>
> `pi5/trajectory.py`의 `predict_landing`은 착지점을 **카메라 원점 기준**으로 돌려주면서
> "카메라가 로봇에 실려 있으므로 그대로 로봇이 이동할 거리"라고 전제한다.
> 이건 **카메라가 로봇 회전 중심에 있을 때만** 참이다.
> 설계상 카메라는 **통 입구 림**(중심에서 약 15 cm)에 두기로 했는데
> ([hardware.md 3.1장](docs/hardware.md#장착-규약-중요)),
> 지금 `pi5/`에는 오프셋 보정이 **아예 없다.**
>
> 15 cm는 통 반경과 같은 크기라 그대로 두면 유효 캐치 반경이 절반으로 줄어든다.
> 자세한 내용은 [`docs/open-questions.md`](docs/open-questions.md).
